...
module tb;
  ...
  // DUT
  ac_range_check dut (
  // TODO MVy: should we keep the default values? Plan to verify with the others? As the TB will be impacted and should adopt a particular technique
  //   .AlertAsyncOn              = ,
  //   .EnableRacl                = ,
  //   .RaclErrorRsp              = ,
  //   .RaclPolicySelVec[NumRegs] =
  // ) (
    .clk_i                    (clk                    ),
    .rst_ni                   (rst_n                  ),
  ...
  initial begin
    ...
    uvm_config_db#(virtual tl_if)::set(null, "*.env.tl_csr_agent*", "tl_csr_vif", tl_csr_if);
    uvm_config_db#(virtual tl_if)::set(null, "*.env.tl_unfilt_agent*", "tl_unfilt_vif", tl_unfilt_if);
    uvm_config_db#(virtual tl_if)::set(null, "*.env.tl_filt_agent*", "tl_filt_vif", tl_filt_if);
