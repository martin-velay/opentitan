...
task ac_range_check_scoreboard::process_tl_access(tl_seq_item item,
  ...
  case (csr_name)
    ...
    "range_base": begin
      if (tl_phase == AddrWrite) begin
        dut_cfg.range_base[csr_idx] =
          get_field_val(ral.range_base[csr_idx].base, item.a_data) << 2;    // 32-bit words
      end
    end
    "range_limit": begin
      if (tl_phase == AddrWrite) begin
        dut_cfg.range_limit[csr_idx] =
          get_field_val(ral.range_limit[csr_idx].limit, item.a_data) << 2;  // 32-bit words
      end
    end
    "range_perm": begin
      if (tl_phase == AddrWrite) begin
        dut_cfg.range_perm[csr_idx].log_denied_access = prim_mubi_pkg::mubi4_test_true_strict(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].log_denied_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].execute_access    = prim_mubi_pkg::mubi4_test_true_strict(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].execute_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].write_access      = prim_mubi_pkg::mubi4_test_true_strict(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].write_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].read_access       = prim_mubi_pkg::mubi4_test_true_strict(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].read_access, item.a_data)));
        dut_cfg.range_perm[csr_idx].enable            = prim_mubi_pkg::mubi4_test_true_strict(
          prim_mubi_pkg::mubi4_t'(
            get_field_val(ral.range_perm[csr_idx].enable, item.a_data)));
      end
    end
    "range_racl_policy_shadowed": begin
      if (tl_phase == AddrWrite) begin
        dut_cfg.range_racl_policy[csr_idx].read_perm =
          get_field_val(ral.range_racl_policy_shadowed[csr_idx].read_perm, item.a_data);
        dut_cfg.range_racl_policy[csr_idx].write_perm =
          get_field_val(ral.range_racl_policy_shadowed[csr_idx].write_perm, item.a_data);
      end
    end
    default: begin
